...
test_that(".valid.BackendMemory.spectra.names", {
    ...
    expect_null(.valid.BackendMemory.spectra.names(list(a=1, b=2)))
...
test_that("backendImportData", {
    ...
    nms <- paste(.vdigest(f)[spd$fileIdx], spd$spIdx, sep="/")
    ...
    expect_equal(lapply(b@spectra, mz),
                 setNames(lapply(pks, function(p)p[,1]), nms))
    expect_equal(lapply(b@spectra, intensity),
                 setNames(lapply(pks, function(p)p[,2]), nms))
})
...
test_that("backendReadSpectra/backendWriteSpectra", {
    ...
    s <- c(new("Spectrum2", mz=1:2, intensity=1:2),
           new("Spectrum2", mz=3:4, intensity=3:4),
