...
## This are the values calculated using the orginal code provided by
## Adriaan Sticker, prior to refactoring to include into MSnbase.
res0 <- structure(c(885.188718025484, 17593.5475899093,
...
test_that("Robust summarisation", {
              data("msnset")
              expect_error(combineFeatures(msnset,
                                           fcol = "ProteinAccession",
                                           fun = "robust"))
              msnset <- filterNA(msnset)
              expect_warning(res <- combineFeatures(msnset,
                                                    fcol = "ProteinAccession",
                                                    fun = "robust"))
              expect_silent(combineFeatures(msnset,
                                            fcol = "ProteinAccession",
                                            fun = "robust",
                                            nIter = 1e3))
              expect_equal(res0, exprs(res))
          })
