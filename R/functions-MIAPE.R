show.MIAPE <- function(object) {
  if (any(c(length(object@instrumentModel)>0,
    ...
    cat("Instrument : \n")
    cat("  Model:",object@object@instrumentModel,"\n")
    cat("  Manufacturer:",object@object@instrumentManufacturer,"\n")
    cat("  Customisations:",object@object@instrumentCustomisations,"\n")
...
msinfo.MIAPE <- function(object) {
  ...
  cat("    (see also individual spectra)\n")
