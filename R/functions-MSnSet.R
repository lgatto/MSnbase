...
##' @param x A \code{matrix} or an \code{MSnSet}.
##' @param fcol The protein grouping feature variable name.
...
##' x <- hyperLOPIT2015ms3r1psm[1:5000, ]
##' x <- droplevels(x)
##' fcol <- "Protein.Group.Accessions"
##' res1 <- agg(x, fcol, fun = max)
##' res2 <- agg(x, fcol, fun = mean)
##' par(mfrow = c(1, 3))
##' plot(res1, log = "y", main = "Single outliers (max)")
##' plot(res2, log = "y", main = "Overall inconsistency (mean)")
##' plot(res1[, "agg_dist"], res2[, "agg_dist"],
##'      xlab = "max", ylab = "mean")
agg <- function(x, fcol, fun) {
    ...
    xx <- split(x, fData(x)[, fcol])
    d <- unlist(lapply(xx, .d))
    nr <- unlist(lapply(xx, nrow))
    ans <- data.frame(agg_dist = d, nb_feats = nr)
