...
the individual spectra are however, in contrast to \Rclass{MSnExp}
objects not kept in memory (in the \Robject{assayData} slot), but are
fetched from the original files on-demand. Because mzML files are
indexed, reading the relevant information for a limited amount of
spectra is very fast and only slightly slower than for
\Rclass{MSnExp}.
