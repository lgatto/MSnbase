...
`data.frames`. Below, we demonstrate how to do that using an `mzid`
file from the `r Biocexptpkg("msdata")` package.
...
The `idx` identifcation object can then be converted into a `data.frame`:

```{r iddf}
iddf <- as(idx, "data.frame")
str(iddf)
```
...
This `data.frame` can be reduced so that row represent unique scans,
which can be done with the `reduce` method:
...
The information for scans that had multiple entries in `iddf` are now
combined as semicolon-separated values:
