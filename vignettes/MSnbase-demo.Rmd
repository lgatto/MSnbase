---
title: "MSnbase: labelled and label-free MS2 data processing, visualisation and quantification"
author: 
- name: Laurent Gatto
- name: Sebastian Gibb
package: MSnbase
...
infrastructure implemented in Bioconductor, in particular `r
...
provided in the `MSnbase-development` vignette.
...
The independent handling of spectra is ideally suited for parallel
processing.  The `quantify` method now performs reporter
peaks quantitation in parallel.  More functions are being updated.

Finally, recent developmenets in version 2 of the package have solved
the memory issue by implementing and *on-disk* version the of data
class storing raw data (*MSnExp*, see section \@ref(sec:msnexp)),
...
`r Biocpkg("MSnbase")` is able to import raw MS data stored in one of the
`XML`-based formats as well as peak lists in the `mfg`
format\footnote{%%
  Mascot Generic Format --
  <http://www.matrixscience.com/help/data\_file\_help.html\#GEN>}

\paragraph{Raw data} The `XML`-based formats, `mzXML`
[@Pedrioli2004], `mzData` [@Orchard2007] and
`mzML` [@Martens2010] can be imported with the
`readMSData` function, as illustrated below (see
`?readMSData` for more details).
...
`msLevel` parameter accordingly.  In this document, we will use
the `itraqdata` data set, provided with `r Biocpkg("MSnbase")`. It
includes feature metadata, accessible with the `fData`
accessor.  The metadata includes identification data for the
\Sexpr{length(itraqdata)} MS2 spectra.

\paragraph{MSnbase 2.0 } The new major version of `r Biocpkg("MSnbase")`
uses a new *on-disk* data storage model (see the
*benchmarking* vignette for more details). The new data backend
is compatible with the orignal *in-memory* model. To make use
of the new infrastructure, read your raw data using
`readMSData2`, rather than `readMSData`. All
...
search engines.  Peak lists in `mgf` format can be imported
with the function `readMgfData` (see
`?readMgfData` for details) to create experiment objects.
Experiments or individual spectra can be exported to an `mgf`
file with the `writeMgfData` methods (see
`?writeMgfData` for details and examples).
...
fractions were loaded using `readMSData` on a 32-cores servers with
128 Gb of RAM.  It took about 90 minutes to read the 14 uncentroided
`mzXML` raw files (4.9 Gb on disk in total) and create a 3.3 Gb raw
data object (an *MSnExp* instance, see next section).  Quantitation of
9 reporter ions (*iTRAQ9* object, see \@ref{sec:reporterions}) for
88690 features was performed in parallel on 16 processors and took 76
minutes.  The resulting quantitation data was only 22.1 Mb and could
easily be further processed and analysed on a standard laptop
computer.

Since verions 1.13.5, parallel support is provided by the
`r Biocpkg("BiocParallel")` and various backends including multicore
...
`r Biocpkg("MSnbase")`. The `MSnbase-io` vignette gives a general
overview of `r Biocpkg("MSnbase")`'s input/ouput capabilites.
...
\item Other meta data, including experimental phenotype, file name(s)
  ...
  implemented as in the *eSet* class and are described in more
  details in their respective manual pages. See `?MSnExp`
  and references therein for additional background information.

  The experiment meta data associated with an *MSnExp*
  experiment is of class *MIAPE*. It stores general
  information about the experiment as well as MIAPE (Minimum
  Information About a Proteomics Experiment) information
  [@Taylor2007, Taylor2008].  This meta-data can be accessed with
  the `experimentData` method.  When available, a summary of
  MIAPE-MS data can be printed with the `msInfo` method.
...
(\Sexpr{paste(paste(head(featureNames(itraqdata)),collapse=", "),",
  ...",sep="")}) and stored in a `environment`. They can be
accessed individually with `itraqdata[["X1"]]` or
`itraqdata[[1]]`, or as a list with
`spectra(itraqdata)`.  As we have loaded our experiment
specifying `msLevel=2`, the spectra will all be of level 2 (or
higher, if available).
...
PRIDE repository and create\footnote{This code chunk is not evaluated
  ...
  code chunks.} an *MSmap* containing all the MS$^1$ spectra
...
The `M` map object can be rendered as a heatmap with
`plot`, as shown on figure \@ref{fig:mapheat}.

\begin{figure}[!ht]
\centering
```{r mapheat, dev='pdf', fig.width=5, fig.height=5}
...
`plot3D` function, as show on figure \@ref{fig:map3d}.

\begin{figure}[!ht]
\centering
```{r map3d1, dev='pdf', fig.width=4, fig.height=4}
plot3D(M)
```
...
(object `M` above) and all intermediate MS$^2$ spectra and
...
\begin{figure}[!ht]
\centering
```{r map3d2, dev='pdf', fig.width=4, fig.height=4}
plot3D(M2)
```
...
\begin{figure}[!ht]
\centering
```{r spectrumPlot, dev='pdf', fig.width=6, fig.height=4, fig.keep='high'}
...
`itraqdata` experiment using the protein accession numbers
...
%% Lets start by extracting all spectra that have the same precursor MZ value than `sp`
%% into a separate experiment, using the `extractPrecSpectra` method.
%% This method takes an `MSnExp` experiment and a precursor MZ value as parameters.
%% ```{r extractPrecSpec,echo=T,cache=F}
%% exp2 <- extractPrecSpectra(raw.experiment,precursorMz(sp))
%% exp2
%% ```

These can then be visualised together by plotting the
*MSnExp* object, as illustrated on figure \@ref{fig:msnexp-plot}.

\begin{figure}[p]
\centering
```{r msnexpPlot, dev='pdf', fig.width=3.7, fig.height=4.5, fig.keep='last'}
...
\paragraph{Customising your plots} {#sec:customplots}

The `r Biocpkg("MSnbase")` `plot` methods have a logical
`plot` parameter (default is `TRUE`), that specifies
...
about \CRANpkg{ggplot2} can be found in [@ggplot2] (especially
chapter 8) and on
\href{http://had.co.nz/ggplot2/}{http://had.co.nz/ggplot2/}.  Finally,
if a plot object has been saved in a variable `p`, it is
possible to obtain a summary of the object with
`summary(p)`.  To view the data frame used to generate the
plot, use `p```data`.
...
The `addIdentificationData` method takes an *MSnExp*
instance (or an *MSnSet* instance storing quantitation data,
see section \@ref{sec:quant}) as first argument and one or multiple
`mzIdentML` file names (as a character vector) as second one and
updates the *MSnExp* feature data using the identification
data read from the `mzIdentML` file(s).
...
\begin{figure}[!ht]
\centering
```{r fragplotfig, dev='pdf', fig.width=5, fig.height=5}
...
Note that `removeNoId` and
`removeMultipleAssignment` methods can also be called on
*MSnExp* instances.
...
The `plot2d` method takes one *MSnExp* instance as
first argument to produce retention time *vs.* precursor MZ
scatter plots. Points represent individual MS2 spectra and can be
coloured based on precursor charge (with second argument
`z="charge"`), total ion count (`z="ionCount"`),
number of peaks in the MS2 spectra `z="peaks.count"`) or, when
multiple data files were loaded, file `z="file"`), as
...
\begin{figure}[htb]
    \includegraphics[width=\linewidth]{./Figures/plot2d-figure.png}
\caption{Illustration of the `plot2d` output. }
 {#fig:plot2d}
\end{figure}

The `plotDensity` method illustrates the distribution of
several parameters of interest (see figure \@ref{fig:plotDensity}).
Similarly to `plot2d`, the first argument is an
*MSnExp* instance.  The second is one of
`precursor.mz`, `peaks.count` or `ionCount`,
whose density will be plotted.  An optional third argument specifies
whether the x axes should be logged.

\begin{figure}[htb]
    \includegraphics[width=\linewidth]{./Figures/plotDensity-figure.png}
\caption{ Illustration of the `plotDensity` output. }
 {#fig:plotDensity}
\end{figure}
...
The `plotMzDelta` method\footnote{The code to generate the
  histograms has been contributed by Guangchuang Yu from Jinan
  University, China.} implements the $m/z$ delta plot from
[@Foster11] The $m/z$ delta plot illustrates the suitability of MS2
...
(`?plotMzDelta`).  Figure \@ref{fig:plotMzDelta-pride12011}
has been generated using the PRIDE experiment 12011, as in
[@Foster11].

\begin{figure}[htb]
  \begin{center}
    \includegraphics[width=.7\linewidth]{./Figures/plotMzDelta-pride12011.pdf}
    \caption{Illustration of the `plotMzDelta` output for
      the PRIDE experiment 12011, as in figure 4A from
      [@Foster11]. }
 {#fig:plotMzDelta-pride12011}
  \end{center}
\end{figure}
...
\begin{figure}[!ht]
```{r spectrum-clean-plot, dev='pdf', fig.width=6, fig.height=3, echo=FALSE, fig.keep='high'}
...
and `clean` methods are illustrated on figure \@ref{fig:preproc}.
...
\begin{figure}[p]
\centering
```{r preprocPlot, dev='pdf', fig.width=3, fig.height=6, echo=FALSE}
grid.newpage()
...
```
\caption{This figure illustrated the effect or the `removePeaks`
  and `clean` methods. The left-most spectrum displays two peaks,
  of max height 3 and 7 respectively. The middle spectrum shows the result
  of calling `removePeaks` with argument `t=3`, which sets
  all data points of the first peak, whose maximum height is smaller or equal
  to `t` to 0. The second peak is unaffected. Calling `clean`
  after `removePeaks` effectively deletes successive 0 intensities from
...
Another useful manipulation method is `trimMz`, that takes
as parameters and *MSnExp* (or a *Spectrum*) and a
numeric `mzlim`. MZ values smaller then `min(mzlim)` or
greater then `max(mzmax)` are discarded. This method is
...
# MS$^2$ isobaric tagging quantitation {#sec:isoquant}
...
The `quantify` method takes the following parameters: an
*MSnExp* experiment, a character describing the quantification
`method`, the `reporters` to be quantified and a
`strict` logical defining whether data points ranging outside
of `mz` $\pm$ `width` should be considered for
quantitation.  Additionally, a progress bar can be displaying when
setting the `verbose` parameter to `TRUE`.  Three
quantification methods are implemented, as illustrated on figure
\@ref{fig:quant-methods}: `trapezoidation` returns the area
under the peak of interest, `max` returns the apex of the peak
and `sum` returns the sum of all intensities of the peak.  See
`?quantify` for more details.
...
\begin{figure}[p]
\centering
```{r quantitationPlot, dev='pdf',fig.width=5, fig.height=5, echo=FALSE}
grid.newpage()
...
\caption{The different quantitation methods are illustrated above.
  Quantitation using `sum` sums all the data points in the
  peaks to produce, for this example,
  \Sexpr{quantify(ssp,iTRAQ4[1],method="sum")[[1]]}, whereas method
  `max` only uses the peak's maximum intensity,
  \Sexpr{quantify(ssp,iTRAQ4[1],method="max")[[1]]}.
  `Trapezoidation` calculates the area under the peak taking
  the full with into account (using `strict=FALSE` gives
...
The `quantify` method returns *MSnSet* objects, that
extend the well-known *eSet* class defined in the
`r Biocpkg("Biobase")` package. *MSnSet* instances are very
similar to *ExpressionSet* objects, except for the experiment
...
features/spectra originally in the *MSnExp* used as parameter
in `quantify` and $m$ is the number of reporter ions, that can
be accessed with the `exprs` method.  The meta data is
directly inherited from the *MSnExp* instance.
...
isobaric tags using the `quantify` method and the
`TMT10` reporter instance. The data on the $x$ axis has been
quantified using `method = "max"` and centroided data (as
generated using ProteoWizard's `msconvert` with vendor
libraries' peak picking); on the $y$ axis, the quantitation method was
`trapezoidation` and `strict = TRUE` (that's important
for TMT 10-plex) and the profile data. We observe a very good
correlation.

\begin{figure}[htb]
  \centering
  \includegraphics[width=.6\linewidth]{./Figures/tmt10comp.pdf}
...
variants.  The `purityCorrect` method applies such
correction to *MSnSet* instances. It also requires a square
matrix as second argument, `impurities`, that defines the
relative percentage of reporter in the quantified each peak.  See
`?purityCorrect` for more details.
...
updated for *MSnSet* without any missing values. Below, we
apply a deterministic minimum value imputation on the `naset`
example data:
...
\item Biologically relevant missing values, resulting from the
  *absence* of the low abundance of ions (below the limit of
...
\begin{figure}[h]
  \centering
  ```{r miximpfig, echo=FALSE, dev='pdf', fig.width=8, fig.width=8}
  x <- impute(naset, "zero")
  exprs(x)[exprs(x) != 0] <- 1
...
\begin{description}
\item{`quantiles`} Applies quantile normalisation
  [@Bolstad03] as implemented in the
  `normalize.quantiles` function of the
  `r Biocpkg("preprocessCore")` package.
\item{`quantiles.robust`} Applies robust quantile
  normalisation [@Bolstad03] as implemented in the
  `normalize.quantiles.robust` function of the
  `r Biocpkg("preprocessCore")` package.
\item{`vsn`} Applies variance stabilisation normalization
  [@Huber2002] as implemented in the `vsn2` function of
  the `r Biocpkg("vsn")` package.
\item{`max`} Each feature's reporter intensity is divided by
  the maximum of the reporter ions intensities.
\item{`sum`} Each feature's reporter intensity is divided by
  the sum of the reporter ions intensities.
\end{description}

See `?normalise` for more methods. A `scale`
method for *MSnSet* instances, that relies on the
`base::scale` function.
...
figure \@ref{fig:cv-plot} reproduces figure 3 of [@Karp2010] that
described the application of vsn on iTRAQ reporter data.

\begin{figure}[p]
\centering
```{r normPlot, dev='pdf', fig.width=5, fig.height=7, echo=FALSE}
...
```
\caption{ Comparison of the normalisation *MSnSet* methods.
...
\begin{figure}[!ht]
\centering
```{r cvPlot, dev='pdf', fig.width=5, fig.height=4, echo=FALSE}
print(p)
```
...
*MSnExp* experiments with the `normalise` method
using the `max` method.  This will rescale all peaks between 0
and 1.  To visualise the relative reporter peaks, one should this
first trim the spectra using method `trimMz` as illustrated
in section \@ref{sec:rawprocessing}, then normalise the *MSnExp*
with `normalise` using `method="max"` as illustrated
above and plot the data using `plot` (figure
...
\begin{figure}[p]
\centering
```{r msnexpNormPlot, dev='pdf', fig.width=5, fig.height=6, echo=FALSE}
print(p)
```
...
`combineFeatures`'s first argument, `object`, is an
instance of class *MSnSet*, as has been created in the section
\@ref{sec:quant} for instance.  The second argument, `groupBy`,
is a `factor` than has as many elements as there are features
in the *MSnSet* `object` argument. The features
corresponding to the `groupBy` levels will be aggregated so
that the resulting *MSnSet* output will have
`length(levels(groupBy))` features. Here, we will combine
...
specified as strings (`fun="mean"`, `fun="median"`,
`fun="sum"`, `fun="weighted.mean"` or
`fun="medianpolish"` to compute respectively the mean, media,
...
`fun=function(x) \{ ... \`}. We will use the `median`
here.
...
# Label-free MS$^2$ quantitation {#sec:lf}
...
`quantify`, as in section \@ref{sec:quant}, this time however
specifying the method of interest, `SIn` (the `reporters`
...
feature meta-data using the default `accession` and `length`
feature variables.
...
\begin{figure}[!ht]
\centering
```{r plotSpectrumSpectrum, dev='pdf', fig.width=6.5, fig.height=6.5}
...
\begin{figure}[!ht]
\centering
```{r hclustcompareSpectra, dev='pdf', fig.width=5, fig.height=6}
...
\begin{figure}[h]
  \begin{center}
    \includegraphics[width=4cm]{./Figures/itraqchem.pdf}
...
`r Biocpkg("MSnbase")` provides, among others, a *ReporterIons*
object for iTRAQ 4-plex that includes the 145 peaks, called
*iTRAQ5*. This can then be used to quantify the experiment as
...
\begin{figure}[!hbt]
\centering
```{r incompdissPlot, dev='pdf', fig.width=5, fig.height=2.5, echo=FALSE, warning=FALSE}
...
Default *MSnSet* feature names (`X1`, `X2`,
\ldots) and sample names (`iTRAQ4.114`, `iTRAQ4.115`,
`iTRAQ4.116`, \ldots) are not informative.  The features and
...
result in an error because data points for the same *feature*
name (say `X1`) and the same *sample name* (say
`iTRAQ4.114`) have different values.  We thus first update the
...
convenience function `updateFeatureNames`.  Note that
...
Lets now create two *MSnSet*s from the same raw data to
...
been used as grouping factor in `combineFeatures`, then
...
\begin{figure}[!ht]
\centering
```{r avg2, dev='pdf', fig.height=5, fig.width=5}
...
```
\caption{PCA plot of the averaged *MSnSet*. The point sizes
...
produces *MSnSet* instances for indvidual acquisitions. The
`r Biocpkg("MSnbase")` infrastructure can subsequently be used to further
combine experiments, as shown in section \@ref{sec:comb2} and apply
*top3* quantitation using the `topN` method.
