...
raw data by setting the `mode` argument to `"onDisk"` (the default in
`"inMemory"`). The new *on-disk* implementation supports several MS
levels in a single raw data object. All existing operations work
...
* `iPQF`: A spectra-to-protein summarisation method integrates peptide
  spectra characteristics and quantitative values for protein
  quantitation estimation. See `?iPQF` and references therein for
  details.
