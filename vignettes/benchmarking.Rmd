...
file for access, which is done only once. On-disk access performance will
obviously depend on the read throughput of the employed hard disk. A comparison
of the data import of the above file from an internal solid state disk and from
an USB3 connected hard disk drive showed only small differences for the
`readMSData2` call (1.07 *vs* 1.36 seconds), while no difference was observed
for accessing individual or all spectra data. Thus, for this particular setup,
performance was about the same for SSD and HDD. This might however not apply to
