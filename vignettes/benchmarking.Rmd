...
in-memory (i.e. pre-loaded and constructred) and on-disk
...
While it takes order or magnitues more time to access the data
on-the-fly rather than a pre-generated spectrum, accessing all spectra
is only marginally slower than accessing all spectra. This is because
most of the time is spent preparing the file for access, which is done
only once. 

Also, it is important to note that 
- Data access does not prohibit interactive usage (about 1/2 second)
- Accessing the data is actually not frequent
- Caching mechanism
- Some operations have been optimised, as for example quantitation, as
