...
time is spent preparing the file for access, which is done only
once. On-disk access performance will obviously depend on the read throughput of
the employed hard disk. In the code chunk below we compared data import and
spectra access speed between an internal solid state disk and an external hard
disk drive connected through USB3.

```{r ssdVsSata, eval=FALSE}
...
It took 1.07 seconds to read the file from SSD and 1.36 seconds to read it from
the external HDD while no difference in performance could be observed for the
subsetting operations. Thus, for this particular setup, performance of data
import from SSD and HDD was about the same. This might however not apply to
setting in which data import is performed in parallel from multiple files.
