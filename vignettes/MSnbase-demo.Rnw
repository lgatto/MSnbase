...
to filter out spectra that match peptides that have been assigned to
multiple proteins. This function uses by default the \code{npsm}
feature variable.
...
The spectral abundance factor (NAF) and the normalised form (NSAF)
...
to proteins (as defined by the \code{npsm} feature variable column)
with the \Rfunction{removeMultipleAssignment} method.

<<labelfree>>=
fData(msexp)[, c("accession", "npsm")]
@
...
\Rfunction{quantity}, as in section \ref{sec:quant}, this time however
