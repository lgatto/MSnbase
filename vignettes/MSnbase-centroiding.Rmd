...
vignette: >
  ...
  %\VignetteKeywords{Mass Spectrometry, MS, MSMS, Proteomics, Metabolomics, Bioinformatics}
...
target m/z value [@Smith:2014di]. The accuracy of that signal depends
...
provides some functionality to perform centroiding of profile MS
...
intensities measured by the mass spectrometer over retention time for
the selected m/z. The upper panel displays the *base peak
chromatogram* (BPC), which represents the maximum signal for each
discrete retention time across the range of m/z values. The horizontal
rows of points in the lower panel indicate the resolution of the mass
...
data point, for the ion in a spectrum (i.e. at a given retention
time). The simplest approach selects the largest intensity for each
mass peak and report its intensity and m/z value. This can be done
using the `pickPeaks` method with default parameters as shown below.

```{r simple-pickPeaks, fig.align = "center", warning = FALSE, fig.cap = "Centroided data for serine."}

serine_cent <- pickPeaks(serine)
...
intensity signals this variation can be substantial. To illustrate
this we plot below the centroided signal for the [M+H]+ ion of
proline.
...
scans and the chromatographic data does not show a nice, regular peak shape. Additional
...
We next apply the simple peak picking on the smoothed data and
subsequently plot the such centroided data for serine.
...
the centroided data of serine (and also the data for proline as can be
seen below).
...
The smoothed centroided data for proline still show considerable
fluctuations of m/z values and poor chromatographic data.

In addition to smoothing the signal in m/z dimension, we can also smooth the signal
along the retention time dimension using the `combineSpectraMovingWindow`
...
Smoothing in retention time dimension improves the chromatographic
peak shape of proline.
...
by passing either `refineMz = "descendPeak"` or `refineMz = "kNeighbors"`
to the `pickPeaks` method. The m/z value of the reported centroid is calculated using
