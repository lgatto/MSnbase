...
vignette: >
  ...
  %\VignetteKeywords{Mass Spectrometry, MS, MSMS, Proteomics, Infrastructure,
  Bioinformatics, magrittr}
...
## Plot the centroided data for proline
data_cent %>%
    filterRt(prol_rtr) %>%
    filterMz(prol_mzr) %>%
    plot(type = "XIC")
...
proline_sg_cent <- data_sg_cent %>%
    filterRt(prol_rtr) %>%
    filterMz(prol_mzr)
proline_sg_cent_mz <- data_sg_cent_mz %>%
    filterRt(prol_rtr) %>%
    filterMz(prol_mzr)
