...
This document is not replacement for the individual manual pages, that  
slots of the \Rpackage{MSnbase} classes. It is a centralised high-level 
...
This virtual class aims coordinate contain mass spectrometry 
data, i.e spectra, and meta data. It is based on the \Robject{eSet} for genomic 
data. The main difference with \Robject{eSet} that the \Robject{assayData} slot is an 
environment containing any number of \Robject{Spectrum} instances (see section \ref{sec:spectrum}).

Two new slots are introduced, namely \Robject{proteomicsData}, that contains 
\Robject{MIAPE} instances (see section \ref{sec:miape}) and 
\Robject{processingData}, that contains \Robject{MSnProcess} instances 
(see section \ref{sec:msnprocess}). The \Robject{annotation} slot has not 
...
%% section
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\Robject{MSnExp}: a class MS experiments}
\label{sec:msnexp}

\Robject{MSnExp} extends \Robject{pSet} to 
an is used to store MS experiments. It does not add any new slots 
...
%% section
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\Robject{MSnSet}: a class quantitative proteomics data}
...
(see section \ref{sec:reporterions}). This thus prompted to directly extend 
the \Robject{ExpressionSet} and add two proteomics-specific annotation slots
introduced in the \Robject{pSet} class, namely \Robject{proteomicsData} for 
\Robject{MIAPE} objects (see section \ref{sec:miape}) and 
\Robject{processingData} for objects of class \Robject{MSnProcess} 
(see section \ref{sec:msnprocess}). The \Robject{annotation} slot, 
inherited from \Robject{eSet} is not used.
...
Any new processing that is implemented should be documented logged here.

It also documents the raw data file from which the data originates (\Robject{files} 
slot) and the \Rpackage{MSnbase} version that was installed when the \Robject{MSnProcess} 
...
Experiment \citep{Taylor2007} \Robject{MIAPE} class is currently
only composed by a free text \Robject{description} attribute. This 
class will be expanded in future versions. 
Note that the MIAPE standard information that overlaps with MIAME 
requirements should directly be documented in the \Robject{experimentData} 
slots (see also note on page \pageref{misc:whymiame}).
...
%% section
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\Robject{Spectrum} \it{et al.}: classes for MS spectra}
\label{sec:spectrum}
...
are implemented with \Robject{ReporterIons}, 
