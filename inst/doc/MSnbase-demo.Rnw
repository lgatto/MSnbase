...
format \citep{Pedrioli2004}. More formats, notably \texttt{mzML} \citep{Martens2010}, 
will be added at a later stage. 
Either one of MS1 or MS2 spectra can be loaded at a time by setting the \texttt{msLevel} parameter accordingly.
...
exp2 <- extractPrecSpectra(experiment,precursorMz(experiment)["X43"])
exp2
@ 

We see that \Sexpr{length(exp2)} spectra have the same MZ value. 
There two spectra can be represented together by plotting the 
\Robject{MSnExp} object (figure \ref{fig:msnexp-plot}).
...
not remove peaks from the spectrum. To effectively remove 0 intensity 
