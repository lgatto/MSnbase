...
See \Rfunction{?readMzXMLData} for more details.
...
In this document, we will use a slightly bigger dataset, \Robject{itraqdata}, provided with 
\Rpackage{MSnbase}. It includes feature metadata, accessible with the \Rfunction{fData} method. 
...
\texttt{itraqdata[["X10"]]} or as a list with \texttt{spectra(itraqdata)}.
