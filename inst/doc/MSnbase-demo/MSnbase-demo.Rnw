...
<<make2exps1, echo=TRUE, cache=FALSE>>=
...
Setting the feature names is critical when multiple experiments are to be combined, as 
this is done using common features as defined by their names (see below).
