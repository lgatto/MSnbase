...
opts_knit$set(stop_on_error = 1L)
...
\begin{figure}[!ht]
<<spectrumPlot, dev='pdf', fig.width=4, fig.height=4, fig.keep='high'>>=
...
<<makeexp12, echo=TRUE, cache=FALSE>>=
...
we combine the specta based on the proteins they have been identified to belong to.
...
<<make2exps2, echo=TRUE, cache=FALSE>>=
set.seed(1)
...
The MS$^E$ pipeline depends on the Bioconductor \Rpackage{synapter} package \citep{synapter} that 
