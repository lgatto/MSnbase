...
Default MSnSet feature names (\Robject{X1}, \Robject{X2}, \ldots) and sample names 
(\Robject{iTRAQ4.114}, \Robject{iTRAQ4.115}, \Robject{iTRAQ4.116}, \ldots) are not very informative.
The features and samples of these anonymous quantitative data-sets should be updated prior 
to additional downstream analysis. 

Lets create two \Robject{MSnSet}s from the same raw data to simulate two different 
independent experiments. As done previously (see section \ref{sec:feataggregation}), 
...
<<make2exps, echo=TRUE, cache=FALSE>>=
set.seed(1)
...
Sample names should also be updated to replace reporter names with relevant identifiers; 
...
because their feature data is not compatible; they share the same feature 
variable labels, i.e. the feature data column names 
(\Sexpr{paste(head(fvarLabels(exp1), n = 3), collapse=", ")}, \ldots), 
but the content is different because the original data was, in particular 
all the spectrum data. Feature data with identical labels (columns in the data frame) 
...
Instead of removing these overlapping and different feature data columns, 
one can use a second convenience function, \Rfunction{updateFvarLabels}, 
to update feature labels based on the experiements variable name and maintain all the metadata.
...
It is now possible to combine \Robject{exp1} and \Robject{exp2} with the 
\Rfunction{combine} method. The new experiment will contain the union of 
...
In summary, when experiments need to be combined, one needs to 
