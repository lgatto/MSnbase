...
print(filename)
raw.experiment <- readMzXMLData(filename, msLevel = 2, verbose=FALSE)
raw.experiment
@

<<experiment-size,echo=FALSE,cache=FALSE>>=
sz <- sum(sapply(assayData(raw.experiment),object.size)) + 
  object.size(raw.experiment)
...
The raw data is composed of the \Sexpr{length(raw.experiment)} MS spectra. 
The spectra are named individually 
(\Sexpr{paste(paste(head(featureNames(raw.experiment)),collapse=", "),", ...",sep="")})
and stored in a \Robject{environment}. They can be accessed individually with 
\texttt{raw.experiment[["X10"]]} or as a list with \texttt{spectra(raw.experiment)}.
...
peaksCount(sp)
head(peaksCount(raw.experiment))
rtime(sp)
head(rtime(raw.experiment))
@ 
...
experiment <- removePeaks(raw.experiment,t=3,verbose=FALSE)
## total ion current
tic(sp)
tic(experiment[["X43"]])
@ 
...
grid()
lines(seq(1,70,by=(70/length(rmed1))),rmed1,col="steelblue",lwd=1)
lines(seq(1,70,by=(70/length(rmed2))),rmed2,col="red"      ,lwd=1)
## legend("topright",
...
\subsection{Purity correction}
\label{sec:purcor}

Impurities in the reporter reagents can bias the results and can be corrected when 
manufacturers provide correction coefficients. 
...
instances. It also requires a square matric as second argument, \Robject{impurities}, 
that defines the repaltive percentage of reporter in the quantified each peak. 
...
impurities <- matrix(c(0.929,0.059,0.002,0.000,
                     ...
                     nrow=4)
qnt2.crct <- purityCorrect(qnt2,impurities)
head(exprs(qnt2))
head(exprs(qnt2.crct))
@ 
...
iTRAQ5
incompdiss <- quantify(raw.experiment,
                       method="trap",
                       reporters=iTRAQ5,
                       strict=FALSE,
                       verbose=FALSE)
