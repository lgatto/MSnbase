...
\subsection{Importing experiments}\label{sec:msnexp}
...
In this document, we will use a slightly bigger data set, \Robject{itraqdata}, provided with 
...
for the MZ values, \Rfunction{intensity} for the intensities, \ldots see the 
...
Note that it is also possible to normalise individual spectra or whole experiments with 
